            ...
            if (status === "Invoice Pending") {
                ...
                oModel.read("/ZP_AISP_POVIM_HEAD", {
                    ...
                    success: function (res) {
                        ...
                        let today = new Date();
                        let formattedDate = `${today.getFullYear()}-${String(today.getDate()).padStart(2, '0')}-${String(today.getMonth() + 1).padStart(2, '0')}`;
                        headData.Invoicedate = formattedDate;
                ...
                oModel.read("/VIMDATA", {
                    ...
                    success: function (res) {
                        ...
                        let today = new Date();
                        // let formattedDate = `${today.getFullYear()}-${String(today.getDate()).padStart(2, '0')}-${String(today.getMonth() + 1).padStart(2, '0')}`;
                        // result.Invoicedate = today;
                        // result._invoiceDateObj = today;
                        // Set headData model
                        const oHeadModel = new JSONModel(result);
                        oHeadModel.setProperty("/cDate", today)
            ...
            sap.m.MessageBox.confirm("Are you sure you want to Submit?", {
                ...
                onClose: function (oAction) {
                    if (oAction === sap.m.MessageBox.Action.OK) {
                        ...
                        if (!invoiveNum || invoiveNum.trim() === "") {
                            MessageBox.warning("Invoice number is required.");
                            that.getView().setBusy(false);
                            return;
                        }

                        if (!invoiceDateObj) {
                            MessageBox.warning("Invoice date is required.");
                            that.getView().setBusy(false);
                            return;
                        }

                        if (!aAttachments || aAttachments.length === 0) {
                            MessageBox.warning("At least one attachment is required.");
                            that.getView().setBusy(false);
                            return;
                        }
